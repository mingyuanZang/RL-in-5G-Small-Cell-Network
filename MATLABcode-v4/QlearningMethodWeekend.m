function [ totalPowerConsumption, Qval, cumulativePowerConsumption] = QlearningMethodWeekend(FBS, n_FBSs,...
...
initial_epsilon = 1;    % Initial Exploration coefficient
    ...
    while(t < MAX_CONVERGENCE_TIME + 1) 
         ...
         [n_UEs UE_location] = UEPara_Week(t);
         n_UEs1 = n_UEs(1,1);
         nUE(t) = n_UEs1;
...
%     grid on, plot(switchingOnRate);
    grid on, plot(cumulativePowerConsumption,'-b');
