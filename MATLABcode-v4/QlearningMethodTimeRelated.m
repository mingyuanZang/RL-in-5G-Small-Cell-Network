function [Normalized_tpt_experienced_per_fbs, totalPowerConsumption, Qval, cumulativePowerConsumption] = QlearningMethodTimeRelated(FBS, n_FBSs,...
        ...
        switchingOffRate(t) = sum(switchingOff(t))/n_FBSs;
         for FBS_i = 1 : n_FBSs
             if (switchingOn(FBS_i) ==1)
                PowerConsumption(FBS_i) = L(t) * (105.6 + 39*Normalized_tpt_FBS(FBS_i));
             else 
                PowerConsumption(FBS_i) = 0;     
             end
         end
        totalPowerConsumption(t) = 750 + 600*(1+switchingOffRate(t)) * (1 - L(t)) * Normalized_tpt_BS + switchingOnRate(t)*n_FBSs*20 + sum(PowerConsumption); %关掉微基站后原来其服务用户转到宏基站中
             ...
             if(nUE(t)>max_nUE)
                     rw = 0;
                      ...
                      elseif switchingOff(FBS_i) ==1
                              rw = PowerConsumption(FBS_i);%基站基础能耗+打开能耗
                      else 
                          rw = 0;
                      end
             end   
...
%     grid on, plot(switchingOnRate);
    figure;
%     hold on; plot(Total_tpt_experienced_per_fbs, 'b');
