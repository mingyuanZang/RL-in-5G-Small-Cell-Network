function [Normalized_tpt_experienced_per_fbs, totalPowerConsumption, Qval, cumulativePowerConsumption] = QlearningMethodMC(FBS, n_FBSs,...
    ...
    %% Calc throughput 
        ...
        for i=1:n_FBSs % Iterate sequentially for each agent in the random order                      
            ...
            while(learning_iteration <= MAX_LEARNING_ITERATIONS)
                ...
                selected_action = randint(1,1,[1 3]);
       ...
       if rem(T,10) == 0
         for FBS_i = 1 : n_FBSs             
            ...
            Qval{FBS_i}(current_action(order(i)), next_action(order(i))) = ...
                Qval{FBS_i}(current_action(order(i)), next_action(order(i))) + ...
                alpha * (rw + gamma * Qval{FBS_i}(next_action(order(i)), next_action(order(i))) - Qval{FBS_i}(current_action(order(i)),current_action(order(i))));
